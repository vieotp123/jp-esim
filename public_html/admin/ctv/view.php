...
if ($id <= 0) { http_response_code(400); echo 'Invalid CTV id'; exit; }
$flash=null;
function load_ctv(int $id): array { $st=db()->prepare('SELECT * FROM ctv_users WHERE id=? LIMIT 1'); $st->execute([$id]); $r=$st->fetch(); if(!$r) { http_response_code(404); echo 'CTV not found'; exit; } return $r; }
...
<div class="card"><a class="btn secondary" href="/admin/ctv/index.php">← Danh sách</a><h2><?= htmlspecialchars((string)$ctv['email']) ?></h2><p>ID #<?= $id ?> · <span class="tag <?= (int)$ctv['status']?'ok':'err' ?>"><?= (int)$ctv['status']?'active':'disabled' ?></span> · Ví <?= htmlspecialchars(format_vnd((int)$ctv['balance'])) ?> · Chiết khấu <?= htmlspecialchars(format_vnd((int)$ctv['discount_per_esim'])) ?></p><p class="muted">Last login: <?= htmlspecialchars((string)($ctv['last_login_at'] ?? '-')) ?> <?= htmlspecialchars((string)($ctv['last_login_ip'] ?? '')) ?></p></div>
...
<div class="card"><h3>Wallet transactions</h3><table><tr><th>Time</th><th>Amount</th><th>Balance</th><th>Reason</th><th>Note</th><th>Admin</th></tr><?php foreach($tx as $r): ?><tr><td><?= htmlspecialchars((string)$r['created_at']) ?></td><td><?= htmlspecialchars(format_vnd((int)$r['amount'])) ?></td><td><?= htmlspecialchars(format_vnd((int)$r['balance_after'])) ?></td><td><?= htmlspecialchars((string)$r['reason']) ?></td><td><?= htmlspecialchars((string)($r['note']??'')) ?></td><td><?= htmlspecialchars((string)($r['admin_user']??'')) ?></td></tr><?php endforeach; ?></table></div>
<div class="card"><h3>Orders</h3><table><tr><th>ID</th><th>Plan</th><th>Charge</th><th>Status</th><th>Error</th></tr><?php foreach($orders as $r): ?><tr><td><?= htmlspecialchars((string)$r['ctv_order_id']) ?></td><td><?= htmlspecialchars((string)$r['plan_name']) ?></td><td><?= htmlspecialchars(format_vnd((int)$r['total_charge'])) ?></td><td><?= (int)$r['status'] ?></td><td><?= htmlspecialchars((string)($r['error_message']??'')) ?></td></tr><?php endforeach; ?></table></div>
<div class="card"><h3>Topups / API / Provider logs</h3><p>Topups: <?= count($topups) ?> · API logs: <?= count($api) ?> · Provider logs: <?= count($provider) ?></p><p><a class="btn secondary" href="/admin/ctv/logs.php?kind=wallet&ctv_id=<?= $id ?>">Xem wallet logs</a> <a class="btn secondary" href="/admin/ctv/logs.php?kind=api&ctv_id=<?= $id ?>">Xem API logs</a> <a class="btn secondary" href="/admin/ctv/logs.php?kind=provider&ctv_id=<?= $id ?>">Xem provider logs</a></p></div>
