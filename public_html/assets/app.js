...
document.addEventListener('click',e=>{ if(e.target.classList.contains('sheet-backdrop')) closeSheet(); if(e.target.matches('[data-copy]')){navigator.clipboard?.writeText(e.target.dataset.copy); toast('Đã sao chép')} if(e.target.matches('[data-close]')) closeSheet(); if(e.target.matches('[data-resume]')) resumeLast(); const rb=e.target.closest('[data-resume-id]'); if(rb){const flow={id:rb.dataset.resumeId,type:rb.dataset.resumeType,t:Date.now()}; localStorage.setItem('jp_last_flow',JSON.stringify(flow)); resumeLast(flow);} const ro=e.target.closest('[data-remove-order]'); if(ro){clearHistoryId(ro.dataset.removeOrder);} });
